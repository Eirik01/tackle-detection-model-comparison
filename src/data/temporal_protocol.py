...
3b. Sliding training set (``build_sliding_training_windows``): stride-1 over
   every clip, so a multi-frame event contributes every overlapping window
   instead of one centered window. Kassab-style caps lifted to window
   granularity: tackle-live uncapped, tackle-replay first-come up to a cap,
   background sampled to a fixed count; residual imbalance is handled by the
   training-time class weights. This matches the dense stride-1 distribution
   used at test time more closely than the centered sampler.
...
def _build_sliding_kassab_seq_caps(
    clip_ids: List[str],
    label_dir: Path,
    seed: int = 0,
    replay_cap: int = 280,
    ...
    """Kassab-faithful sequence-level capping, exploded to stride-1 windows.

    See ``build_sliding_training_windows(balance='kassab_seq_caps', ...)`` for
    the user-facing description.
    """
    rng = np.random.default_rng(seed)

    live_seqs: List[Tuple[str, int, int]] = []     # (clip_id, s_5, e_5)
    replay_seqs: List[Tuple[str, int, int]] = []
    bg_seqs: List[Tuple[str, int, int]] = []       # (clip_id, s, e_exclusive)
    labels_cache: Dict[str, np.ndarray] = {}
    valid_range_cache: Dict[str, Tuple[int, int]] = {}

    for clip_id in clip_ids:
        path = label_dir / f"{clip_id}.json"
        labels_5 = labels_at_5fps(build_frame_labels(path))
        n5 = len(labels_5)
        min_c, max_c = _valid_center_range(n5)
        if max_c < min_c:
            continue
        labels_cache[clip_id] = labels_5
        valid_range_cache[clip_id] = (min_c, max_c)

        events = extract_events_5fps(path)
        for cls, s_5, e_5 in events:
            if cls == TACKLE_LIVE:
                live_seqs.append((clip_id, s_5, e_5))
            elif cls == TACKLE_REPLAY:
                replay_seqs.append((clip_id, s_5, e_5))
        for s, e in _eligible_background_segments(n5, events):
            bg_seqs.append((clip_id, s, e))

    if not live_seqs or not replay_seqs or not bg_seqs:
        raise ValueError(
            "Empty sequence pool in the given clips "
            f"(live={len(live_seqs)}, replay={len(replay_seqs)}, bg={len(bg_seqs)}); "
            "cannot build a sequence-capped sliding training set. Check the split."
        )

    # Apply caps at the sequence level. Live is uncapped (matches Kassab's
    # all-live behaviour). Replay is first-come in data order; default
    # replay_cap=280 equals the full TACDEC replay-sequence count, so the
    # cap does not bind under default settings -- it is exposed only for
    # safety / experiments on smaller splits. Background is sampled
    # uniformly without replacement to bg_count.
    replay_kept = replay_seqs[:replay_cap]
    if len(bg_seqs) > bg_count:
        idx = rng.choice(len(bg_seqs), size=bg_count, replace=False)
        bg_kept = [bg_seqs[i] for i in idx]
    else:
        bg_kept = list(bg_seqs)

    windows: List[Tuple[str, int, int]] = []

    def _explode_event_seq(cls: int, clip_id: str, s_5: int, e_5: int) -> None:
        labels_5 = labels_cache[clip_id]
        min_c, max_c = valid_range_cache[clip_id]
        lo = max(s_5, min_c)
        hi = min(e_5, max_c)
        for c in range(lo, hi + 1):
            if int(labels_5[c]) == cls:
                windows.append((clip_id, c, cls))

    for clip_id, s_5, e_5 in live_seqs:
        _explode_event_seq(TACKLE_LIVE, clip_id, s_5, e_5)
    for clip_id, s_5, e_5 in replay_kept:
        _explode_event_seq(TACKLE_REPLAY, clip_id, s_5, e_5)
    for clip_id, s, e in bg_kept:
        # Every center in [s, e) is eligible by construction of
        # _eligible_background_segments (>= EVENT_GAP_5FPS from any event,
        # inside a run >= MIN_BG_SEG_5FPS, valid window center).
        for c in range(s, e):
            windows.append((clip_id, c, BACKGROUND))

    if not windows:
        raise ValueError(
            "Sequence-capped sliding produced zero windows; check the split / "
            "the cap parameters."
        )
...
def build_sliding_training_windows(
    clip_ids: List[str],
    label_dir: str | Path,
    seed: int = 0,
    ...
    """Sliding-window training set at 5 FPS / W=10.

    Stride-1 sliding over each clip: every valid window center becomes a
    candidate, labelled by its center 5-FPS frame (same rule as eval). Unlike
    ``build_balanced_windows`` (one window per event), a multi-frame event here
    contributes every overlapping window. Three balancing modes:

    ``balance='kassab_caps'`` (default) — Kassab subsample lifted to window
    granularity. Foreground-heavy; relies on training-time class weights to
    absorb the residual imbalance:
      * tackle-live   : every live-center window kept, uncapped.
      * tackle-replay : replay-center windows kept in clip/data order up to
                        ``replay_cap`` windows total (first-come, mirroring
                        cell 7's 280 cap on replay sequences).
      * background    : pool of eligible background-center anchors (>= EVENT_GAP_5FPS
                        from any event frame, inside a run >= MIN_BG_SEG_5FPS), from
                        which ``bg_count`` are sampled uniformly without replacement.

    ``balance='kassab_seq_caps'`` — sequence-level cap, faithful to Kassab
    TempTAC's ``extract_tackle_sequences_and_undersample``. Caps are applied
    on whole *sequences* (not windows); each kept sequence is then exploded
    into every stride-1 window that fits inside it:
      * tackle-live   : every live event sequence kept (uncapped).
      * tackle-replay : kept in data order up to ``replay_cap`` sequences
                        (default 280 = full TACDEC count, so the cap does
                        not bind).
      * background    : eligible background segments (same eligibility rule
                        as ``kassab_caps``) sampled to ``bg_count`` sequences
                        uniformly without replacement (default 500, matching
                        Kassab's ``max_tackles=500``).
      Each kept event sequence contributes one window per 5-FPS center in
      its 5-FPS extent whose center-frame label equals the sequence class
      (boundary-clamped centers off the event are dropped, matching the
      strict center-label rule in ``build_balanced_windows``). Each kept
      bg segment contributes one window per eligible center inside it.

    ``balance='balanced'`` — fully class-balanced. Every class is uniformly
    downsampled (without replacement, seeded) to ``N = min(|live|, |replay|, |bg|)``.
    Live is typically the binding class (smallest of the three sliding pools), so
    in practice every class ends up at the live-pool size and class weights are
    trivially 1.0. ``replay_cap`` and ``bg_count`` are ignored.

    Returns a shuffled list of ``(clip_id, center_frame_5fps, class)`` tuples.

    Raises if any class would be empty.
    """
    if balance not in ("kassab_caps", "balanced", "kassab_seq_caps"):
        raise ValueError(
            f"balance must be 'kassab_caps', 'kassab_seq_caps' or 'balanced', "
            f"got {balance!r}"
        )
    if balance == "kassab_seq_caps":
        return _build_sliding_kassab_seq_caps(
            clip_ids, Path(label_dir), seed=seed,
    ...
    for clip_id in clip_ids:
        ...
        centers = np.arange(min_c, max_c + 1, dtype=np.int64)
        center_cls = labels_5[centers]
        for c in centers[center_cls == TACKLE_LIVE]:
            live_pool.append((clip_id, int(c)))
        for c in centers[center_cls == TACKLE_REPLAY]:
            replay_pool.append((clip_id, int(c)))

        events = extract_events_5fps(path)
        for s, e in _eligible_background_segments(n5, events):
            for c in range(s, e):  # every eligible center in the run
                bg_pool.append((clip_id, c))

    if not live_pool or not replay_pool or not bg_pool:
        raise ValueError(
            "Empty class pool in the given clips "
            f"(live={len(live_pool)}, replay={len(replay_pool)}, bg={len(bg_pool)}); "
            "cannot build a sliding training set. Check the split."
        )

    def _subsample(pool, n):
        if len(pool) == n:
            return list(pool)
    ...
    if balance == "kassab_caps":
        if len(bg_pool) < bg_count:
            raise ValueError(
                f"Only {len(bg_pool)} eligible background windows "
                f"(>= {MIN_BG_SEG_5FPS}-frame runs, >= {EVENT_GAP_5FPS}-frame gap from "
                f"events); need bg_count={bg_count}."
            )
        live_sel = list(live_pool)
        replay_sel = replay_pool[:replay_cap]  # first-come cap in data order
        bg_sel = _subsample(bg_pool, bg_count)
    else:  # balance == "balanced"
        n = min(len(live_pool), len(replay_pool), len(bg_pool))
        live_sel = _subsample(live_pool, n)
        replay_sel = _subsample(replay_pool, n)
        bg_sel = _subsample(bg_pool, n)

    windows = (
        [(c, k, TACKLE_LIVE) for c, k in live_sel]
        + [(c, k, TACKLE_REPLAY) for c, k in replay_sel]
        + [(c, k, BACKGROUND) for c, k in bg_sel]
    )
    order = rng.permutation(len(windows))
...
def build_sliding_windows(
    ...
    """Per-clip stride-1 W=10 sliding windows at 5 FPS for evaluation.

    Returns ``{clip_id: {centers, center_labels, frame_labels, events}}``,
    enough to compute all three metric tracks downstream:
      * frame-level balanced (re-sample from ``centers`` matching the train-set rule),
      * frame-level unbalanced (every center over every test clip),
      * event-level (collapse predicted center labels into runs and match to ``events``).
    """
    label_dir = Path(label_dir)
    out: Dict[str, Dict] = {}
    for clip_id in clip_ids:
        ...
        if max_c < min_c:
            ...
            center_labels = np.array([], dtype=np.int64)
        else:
            centers = np.arange(min_c, max_c + 1, dtype=np.int64)
            center_labels = labels_5[centers].astype(np.int64)

        out[clip_id] = {
            "centers": centers,
            "center_labels": center_labels,
            "frame_labels": labels_5.astype(np.int64),
            "events": events,
        }
    return out
